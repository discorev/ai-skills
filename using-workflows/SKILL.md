...
## How to communicate
Inter-agent messaging should always be done through the `agent-comms` plugin. Invoke the skill before spawning any sub-agent or workflow and follow it. This is the primary comms method and fully replaces `SendMessage`, providing a safe way to communicate.
...
- Keep review agents on-task: scope their prompts to the changed surface. Reviewers have previously drifted into node_modules and dead code, which wastes expensive tokens.
